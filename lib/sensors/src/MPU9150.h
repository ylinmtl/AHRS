...
/**
 * @brief MPU9150 driver (accel+gyro+temp + AK8975 mag via internal I2C master).
 *
 * Scales to:
 *  - Accel: G (±16 g -> 2048 LSB/g)
 *  - Gyro: dps (±2000 dps -> 16.4 LSB/dps)
 *  - Mag:  uT using AK8975 ASA-adjusted counts * 0.3 uT/LSB (datasheet)
 *  - Temp: C using (raw/340 + 36.53)
 *
 * Uses robust mag path:
 *  - SLV0 streams ST1..ST2 into EXT_SENS_DATA
 *  - SLV4 blocking PD->SINGLE trigger every ~12 ms
 */
...
private:
    ...
    // Mag sensitivity adjustment (ASA) factors per axis
    ...
    // Internal-master clock (you’re at 400 kHz per your last change)
    ...
    static constexpr float MAG_uT_PER_LSB  = 0.3f;    // AK8975 typ (before ASA adj). See note.
    ...
    bool readASA();                 // read ASAX/ASAY/ASAZ from fuse ROM, compute _asaAdj*
    void triggerMagIfDue(uint32_t now_us); // PD->SINGLE every ~12 ms
