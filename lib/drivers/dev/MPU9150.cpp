...
bool MPU9150::enableI2CMaster() {
    if (!_bus.writeRegister(_mpu, REG_INT_PIN_CFG, 0x00)) return false;
    ...
    _bus.writeRegister(_mpu, REG_I2C_MST_DELAY, 0x03);
    return true;
}

bool MPU9150::probeMagWIA() {
    if (!_bus.writeRegister(_mpu, REG_I2C_SLV0_ADDR, (uint8_t)(0x80 | _mag))) return false;
    if (!_bus.writeRegister(_mpu, REG_I2C_SLV0_REG,  AK_REG_WIA))              return false;
    if (!_bus.writeRegister(_mpu, REG_I2C_SLV0_CTRL, 0x81))                    return false;
    delay(8);
    uint8_t id=0;
...
bool MPU9150::slv4ReadBlocking(uint8_t dev, uint8_t reg, uint8_t& data, uint32_t timeout_us) {
    ...
    while ((uint32_t)(micros() - t0) < timeout_us) {
        uint8_t st=0;
        if (!readBlock(_mpu, REG_I2C_MST_STATUS, &st, 1)) return false;
        if (st & 0x40) { if (st & 0x10) return false; break; }
    }
